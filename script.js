...
const parentsLine = (parents, name) => { if (!parents) return ''; const names = [parents.father, parents.mother].filter(Boolean).map(n => `<strong class="invitation-name">${esc(n)}</strong>`).join(' · '); return `${names}의 <span class="invitation-child">${esc(parents.child || '')}</span> <strong class="invitation-name">${esc(givenName(name))}</strong>`; };
