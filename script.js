...
const dday = iso => { const wedding = new Date(iso); if (Number.isNaN(wedding)) return ''; const now = new Date(); const inKorea = d => new Date(d.toLocaleString('en-US', { timeZone: 'Asia/Seoul' })); const a = inKorea(now); const b = inKorea(wedding); a.setHours(0, 0, 0, 0); b.setHours(0, 0, 0, 0); const days = Math.round((b - a) / 86400000); return days === 0 ? 'D-DAY' : days > 0 ? `D-${days}` : `D+${Math.abs(days)}`; };
...
<section class="section info reveal"><p class="section-label">WEDDING DAY</p><h2>${esc(when.display)}<br>${esc(when.weekday)} ${esc(when.time)}</h2>${weddingCalendar(when)}<strong class="dday">${dday(w.date)}</strong></section>
