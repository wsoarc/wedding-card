...
function page(d) {
  ...
  const SECTION_DEFAULTS = {
    weddingDay: { label: 'WEDDING DAY' },
    gallery: { label: 'GALLERY', title: '우리의 순간' },
    location: { label: 'LOCATION', title: '오시는 길' },
    accounts: { label: 'WITH LOVE', title: '마음 전하실 곳' },
    guestbook: { label: 'GUESTBOOK', title: '방명록' }
  };
  // invitation처럼, 각 섹션 객체(d.weddingDay, d.gallery, d.location, d.accounts, d.guestbook)에 직접 label/title을 둡니다.
  const sec = (key) => ({ ...SECTION_DEFAULTS[key], ...(d[key] || {}) });
  return `
